...
;[purpose] to show fibonacci sequence that is lenght input number
...
(define (fib n)
  (cond
    ...
    [(= n 1) (list 1)]
    [(= n 2) (list 1 1)]
    [else (append (fib (- n 1)) (list(+ (first(endE (fib (- n 2)) (- n 2))) (first(endE (fib (- n 1)) (- n 1))))))]
    )
  )

(define (endE targetList index)
  (cond
    [(= index 1) targetList]
    [else (endE (rest targetList) (- index 1))]
    )
  )
...
;Time taken: 3
(define-type Vehicle
  [Bicycle (wheels (and/c integer? (not/c negative?)))]
  [Car (wheels (and/c integer? (not/c negative?)))
       (windows (and/c integer? (not/c negative?)))]
  [Airplane (wheels (and/c integer? (not/c negative?)))
            (windows (and/c integer? (not/c negative?)))
            (engines (and/c integer? (not/c negative?)))]
  )
...
(define (vehicle-tax theVehicle wheelTax windowTax engineTax)
  (type-case Vehicle theVehicle
    [Bicycle (wheels) (* wheels wheelTax)]
    [Car (wheels windows) (+ (* wheels wheelTax) (* windows windowTax))]
    [Airplane (wheels windows engines) (+ (* engines engineTax)
                                          (+ (* wheels wheelTax) (* windows windowTax)))]
    )
  )
(test (vehicle-tax (Airplane 5 5 5) 10 10 10)150)
(test (vehicle-tax (Bicycle 3) 150 0 0) 450)
(test (vehicle-tax (Car 4 2) 160 170 0) 980)
