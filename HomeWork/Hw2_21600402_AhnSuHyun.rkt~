...
(define (freeFinder pwae bound-id)
  (type-case PWAE pwae
    [num (n) pwae]
    [postfix (l r opType) (postfix (freeFinder l bound-id) (freeFinder r bound-id) opType)] ;  (postfix (freeFinder l bound-id) (freeFinder r bound-id) opType)
    [substitute (i v e k) (substitute i (freeFinder v bound-id)
                                      (if (symbol=? i bound-id) e (freeFinder e bound-id))
                                      k
                                      )]
    [id (s) (if (symbol=? s bound-id) (num 77) pwae)]
    [else '()]
    )
  )
